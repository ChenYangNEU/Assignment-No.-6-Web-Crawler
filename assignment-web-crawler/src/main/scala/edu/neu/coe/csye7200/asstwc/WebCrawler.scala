...
object WebCrawler extends App {
    ...
    def wget(url: URL)(implicit ec: ExecutionContext): Future[Seq[URL]] = {
        ...
        def getURLs(ns: Node): Seq[Try[URL]] = (ns \\ "a").map { web =>
            val ref = web.attribute("href").getOrElse("").toString
            Try(new URL(url, ref)).flatMap(validateURL)
          }
// END SOLUTION
...
// TO BE IMPLEMENTED 
      for {
        uContent <- getURLContent(url)
        webs <- MonadOps.asFuture(getLinks(uContent))} yield webs
        // END SOLUTION
    }
